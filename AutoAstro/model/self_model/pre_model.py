...
def test_model(model, X_test_tensor, Y_test, scaler_y, label_name):
    ...
    for index,sub_rmse in enumerate(rmse):
        ...
        plt.plot(Y_test_actual[:, index], label=f'Actual {label_name[index]}', alpha=0.7)
        plt.plot(y_pred[:, index], label=f'Predicted {label_name[index]}', alpha=0.7)
        plt.legend()
        plt.title(f'{label_name[index]} Prediction')
        plt.savefig(f'{save_dir}/result_{sanitize_filename(label_name[index])}.png')
        plt.close()
...
def sanitize_filename(filename):
    ...
    # Windows保留文件名: CON, PRN, AUX, NUL, COM1-COM9, LPT1-LPT9
