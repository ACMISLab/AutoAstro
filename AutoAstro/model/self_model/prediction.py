...
data = pd.read_csv("/mnt/zfy/astro/AutoAstro/Data/ts_data/Loamst_Pre.csv") #加载数据
drop_columns = ["Type", "Teff", "logg", "[Fe/H]"]
X = data.drop(columns=drop_columns).values # 只保留 flux 作为特征输入
label_name = ["Teff", "logg", "[Fe/H]"]
Y = data[label_name].values # 目标输出

# 2.开启gpu
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# 3.数据编码和向量转换
X_scaled, Y_scaled, scaler_x, scaler_y = preprocess_data(X, Y) # 数据标准化
X_train, X_test, Y_train, Y_test= split_data(X_scaled, Y_scaled) # 数据集划分
X_train_tensor, X_test_tensor, Y_train_tensor, Y_test_tensor = prepare_tensors(X_train, X_test, Y_train, Y_test, device)

# 4.训练配置
batch_size = 4096 # 默认batch_size大小微4096
model_type = "LSTM" # 可修改为 "GRU", "Transformer", "RNN"
epochs=300
patience=30
learning_rate=adjust_learning_rate(batch_size=batch_size)
write_md(text=f"Training Configuration:\nBatch Size: {batch_size}, Model Type: {model_type}, Epochs: {epochs}, Early Stopping Patience: {patience}, Learning rate: {learning_rate}") # 训练配置记录
...
#
#     # 1.加载数据集 第一列为label标签，其余列为特征列
#     data = pd.read_csv("/ai/astro/AutoAstro/Data/ts_data/output.csv") #加载数据
#     drop_columns = ["Type", "Teff", "logg", "[Fe/H]"]
#     X = data.drop(columns=drop_columns).values # 只保留 flux 作为特征输入
#     Y = data[["Teff", "logg", "[Fe/H]"]].values # 目标输出
#
#     # 2.开启gpu
#     device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
#
...
#     # 5.模型训练与测试
#     train_model(model, train_loader, test_loader, criterion, optimizer,epochs=epochs,patience=patience)
#     test_model(model, X_test_tensor, Y_test, scaler_y)
#
#
#
