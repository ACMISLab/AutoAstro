...
# 1. Load dataset with selected features and target column
data = pd.read_csv("/ai/astro/AutoAstro/Data/ts_data/plasticc.csv")
# Select specified features and target
flux_columns = [f'fulx_{i}' for i in range(28)]  # flux_0 to flux_27
selected_features = flux_columns + ['host_specz', 'host_photoz', 'mwebv']
...
batch_size=1024  # Reduced batch size for better memory management with flux data
model_type = "Transformer"  # Changed to Transformer for better handling of sequential flux data
epochs=150  # Increased epochs for potentially better convergence
patience=15  # Increased patience for early stopping
warmup_epochs = 10  # Increased warmup for Transformer
